...
void PlotAlignmentValidation::plotSubDetResiduals(bool plotNormHisto,unsigned int subDetId)
{
  ...
  switch (subDetId){
  case 1 : histoName+="BPIXBarrel_0";break;
  case 2 : histoName+="FPIXEndcap_1";break;
...
void PlotAlignmentValidation::plotSS( const std::string& options, const std::string& residType )
{
  ...
  for(int i=0; i<hs->GetHists()->GetEntries(); i++) {
    TF1* fitFunc = new TF1("fit"+i,"[0]",-1.2,1.2);
    TH1* histo = (TH1*)hs->GetHists()->At(i);
    histo->Scale(10000.0);

    // Additional style adjustments
    histo->Fit(fitFunc,"QNR");
    fitFunc->SetLineColor(histo->GetLineColor());
    fitFunc->SetLineStyle(histo->GetLineStyle());
    fitFunc->SetLineWidth(2);
    fitFunc->Draw("same");
  }

  float yMax=50.0, yMin=-50.0;
  if(iSubDet==3) {yMax = 6; yMin = -6;}
  // if(iSubDet==5) {yMax = 30; yMin = -30;}
  if(iSubDet==5) {yMax = 30; yMin = -30;}
   hs->SetMaximum(yMax);
   hs->SetMinimum(yMin);
...
//  else if (variable == "medianX") {   plotinfo.nbins = 50;  plotinfo.min = -0.005; plotinfo.max = 0.005; }
  else if (variable == "medianX") {   plotinfo.nbins = 20;  plotinfo.min = -0.001; plotinfo.max = 0.001; }
//  else if (variable == "medianY") {   plotinfo.nbins = 50;  plotinfo.min = -0.005; plotinfo.max = 0.005; }
  else if (variable == "medianY") {   plotinfo.nbins = 20;  plotinfo.min = -0.001; plotinfo.max = 0.001; }
...
void  PlotAlignmentValidation::setHistStyle( TH1& hist,const char* titleX, const char* titleY, int color)
{
  ...
  if /*( titelYAxis.Contains("meanX") )titel_Yaxis<<"#LTx'_{pred}-x'_{hit}#GT[#mum]";
  else if ( titelYAxis.Contains("rmsX") )titel_Yaxis<<"RMS(x'_{pred}-x'_{hit})[#mum]";
  ...
  else if( titelYAxis.Contains("meanLocalX") )titel_Yaxis<<"#LTx_{pred}-x_{hit}#GT[#mum]";
  else if( titelYAxis.Contains("rmsLocalX") )titel_Yaxis<<"RMS(x_{pred}-x_{hit})[#mum]";
...
{
  h = (TH1F*)multiplyXaxis(h, 10000.0);
  ...
  if (direction != 0 && plotinfo.plotSplits && !plotinfo.plotPlain) { 
    ...
    linestyle = (linestyle + linestyleplus) + 1;    
  }
  ...
  if (plotinfo.variable == "medianX" || plotinfo.variable == "meanX" ||
    ...
    if (useFit_) {
      ...
      legend << "#mu = " << h->GetMean(1) << " #mum, rms = " << h->GetRMS(1) << " #mum";
    }
...
//------------------------------------------------------------------------------
TH1* PlotAlignmentValidation::multiplyXaxis(const TH1 *h, double factor)
{ 
  if (!h) return 0;
  if (h->GetXaxis()->IsVariableBinSize()) {
    std::cerr << "multiplyXaxis: variable bin size of input hist "
              << h->GetName() << " not correctly treated" << std::endl;
  }
  TH1 *hNew = 0;
  if (h->InheritsFrom(TH1F::Class())) {
    // would have to check for variable binning!!
    hNew = new TH1F(Form("%s_multX", h->GetName()),
                    h->GetTitle(), h->GetNbinsX(),
                    h->GetXaxis()->GetXmin() * factor,
                    h->GetXaxis()->GetXmax() * factor);
  } else if (h->InheritsFrom(TH1D::Class())) {
    // would have to check for variable binning!!
    hNew = new TH1D(Form("%s_multX", h->GetName()),
                    h->GetTitle(), h->GetNbinsX(),
                    h->GetXaxis()->GetXmin() * factor,
                    h->GetXaxis()->GetXmax() * factor);
  } else {
    std::cerr << "multiplyXaxis: input hist " << h->GetName()
              << "neither TH1F nor TH1D!\n" << std::endl;
    return 0;
  }

  // copy bin contents - and errors
  for (Int_t iBin = 0; iBin <= h->GetNbinsX() + 1; ++iBin) {
  ...
  h->TAttFill::Copy(*hNew);
  h->TAttLine::Copy(*hNew);
  h->TAttMarker::Copy(*hNew);

  // what about various offsets, sizes etc.? 
  return hNew;
}
